...
| `DEVICE_TREE_URL` | Device tree address | https://github.com/TribalChi3f/android_device_tecno_TECNO-KM7.git |
| `DEVICE_TREE_BRANCH` | Device branch that you want to use for build (typically corresponds to the manifest branch) | main |
| `DEVICE_PATH` | Device tree location for syncing, relative to workspace root (usually listed as "LOCAL_PATH" or "DEVICE_PATH" in BoardConfig.mk) | device/tecno/TECNO-KM7 |
| `DEVICE_NAME` | Model name (same as twrp_`<TECNO_KM7>`.mk from device tree) | TECNO-KM7 |
| `DEVICE_MAKEFILE` | Name of device-specific makefile from tree (format: `<twrp>_<TECNO-KM7>`) | twrp_TECNO-KM7
